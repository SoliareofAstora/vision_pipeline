...
class BacteriaBagWithCoord(Dataset):
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        return self.bag_list[idx], self.label_list[idx], self.coordinates[idx]
...
def main():
    labels = pd.read_csv('/workspace/eeml/bakterie/labels.csv')
    patches = pd.read_csv('/workspace/eeml/bakterie/patches.csv')
    features = torch.load('/workspace/eeml/bakterie/2020_04_10_features.pt')
    dataset = BacteriaBag(labels,
                          patches,
                          features,
                          'first_split')
    dataloader = DataLoader(dataset, batch_size=4, shuffle=False, collate_fn=image_collate)
